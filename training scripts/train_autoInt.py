...
print('Training AutoInt model with inner dim: %s...'%(INNER_DIM))
model = AutoInt(linear_feature_columns=linear_feature_columns, dnn_feature_columns=dnn_feature_columns,task='binary',l2_reg_embedding=1e-5, device=device, embd_inner_dim = INNER_DIM)
...
model.fit(train_model_input, train_model_labels,batch_size=BATCH, epochs=args.epoch, validation_split=0.0, verbose=2, amp_enable= False)
pred_ans = model.predict(test_model_input, BATCH)
print("Performance of unfactorized model:")
