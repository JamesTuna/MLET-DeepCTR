...
def create_embedding_matrix(feature_columns, init_std=0.0001, linear=False, sparse=False, device='cpu',inner_dim=None):
    ...
    if inner_dim == None:
        # not using factorization technique
        ...
        {feat.embedding_name:
            nn.Sequential(
                nn.Embedding(feat.vocabulary_size, inner_dim, sparse=sparse),
                nn.Linear(inner_dim, feat.embedding_dim if not linear else 1,bias=False)
            )
            for feat in
            sparse_feature_columns + varlen_sparse_feature_columns}
         )
    ...
    if inner_dim == None:
        for tensor in embedding_dict.values():
            ################################# modified larvar #################################
            n,_ = tensor.weight.size()
            nn.init.uniform_(tensor.weight, a=-np.sqrt(1/n), b=np.sqrt(1 / n))
            ################################# modified larvar #################################
    else:
        for name,para in embedding_dict.named_parameters():
            if name.endswith('weight'):
                ################################# modified larvar #################################
                n,_ = para.size()
                nn.init.uniform_(para, a=-np.sqrt(1/n), b=np.sqrt(1 / n))
                ################################# modified larvar #################################
    ################################# modified larvar #################################
    for l in embedding_dict.modules():
        if isinstance(l,nn.Linear):
            nn.init.normal_(l.weight, mean=0.0, std=0.5)
    ################################# modified larvar #################################
